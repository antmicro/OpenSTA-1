...
Search::findPathGroupForInternalPath(const InputRegisterTimingPath *timing_path) const
{
  ...
  if (stringEq(timing_path->path_group_name.c_str(), PathGroups::pathDelayGroupName())) {
    return search_->pathGroups()->pathDelayGroup(min_max);
  }

  if (stringEq(timing_path->path_group_name.c_str(), PathGroups::gatedClkGroupName())) {
  ...
  if (stringEq(timing_path->path_group_name.c_str(), PathGroups::unconstrainedGroupName())) {
